...
EOF
        ...
        # Ubuntu 24.04+ often uses deb822 /etc/apt/sources.list.d/ubuntu.sources; updating only sources.list is ignored.
        if [ -f /etc/apt/sources.list.d/ubuntu.sources ]; then
            local ubuntu_apt_uri="${mirror_url}/ubuntu"
            if command -v perl &>/dev/null; then
                sudo UBUNTU_APT_URI="$ubuntu_apt_uri" perl -i -pe 's/^URIs:\s*.*/URIs: $ENV{UBUNTU_APT_URI}/' /etc/apt/sources.list.d/ubuntu.sources
            else
                sudo sed -i "s#^URIs:.*#URIs: ${ubuntu_apt_uri}#" /etc/apt/sources.list.d/ubuntu.sources
            fi
            success "Updated URIs in /etc/apt/sources.list.d/ubuntu.sources (deb822)."
            info "Left /etc/apt/sources.list unchanged to avoid duplicate definitions with ubuntu.sources."
        else
            sudo tee /etc/apt/sources.list > /dev/null <<EOF
# Generated by HyperInit
...
# Restart OpenSSH after config change. Debian/Ubuntu use ssh.service; RHEL/Fedora use sshd.service.
_restart_openssh() {
    if command -v systemctl &>/dev/null; then
        if systemctl cat ssh.service &>/dev/null; then
            run_task "Restarting SSH service" sudo systemctl restart ssh
            return $?
        fi
    ...
    if command -v service &>/dev/null; then
        if service ssh status &>/dev/null 2>&1; then
            run_task "Restarting SSH service" sudo service ssh restart
            return $?
        fi
        if service sshd status &>/dev/null 2>&1; then
            run_task "Restarting SSH service" sudo service sshd restart
            return $?
        fi
    fi
    error "Could not find ssh.service or sshd.service. Config was written; restart SSH manually (e.g. sudo systemctl restart ssh)."
    return 1
}
...
harden_ssh() {
    ...
    if [ -f /etc/ssh/sshd_config ]; then
        ...
        if _restart_openssh; then
            success "SSH hardened: Root login disabled."
        else
            warn "sshd_config was updated but the SSH daemon may still be running with the old configuration."
        fi
    else
